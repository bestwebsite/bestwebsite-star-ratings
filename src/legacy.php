...
if (! function_exists('Bestwebsite_star_ratings')) {
    function Bestwebsite_star_ratings($idOrPost = null)
    {
        $id = null;
        ...
        return Bestwebsite\StarRating\response(compact('id'), false);
    }
}

if (! function_exists('Bestwebsite_star_ratings_get')) {
    function Bestwebsite_star_ratings_get($limit = 5, $taxonomyId = null, $offset = 0)
    {
        global $wpdb;
        $postsTable = $wpdb->posts;
        $postMetaTable = $wpdb->prefix.'postmeta';
        $base = get_option(Bestwebsite\StarRating\prefix('stars')) / 5;
        ...
        $queryConditions = "
            WHERE
                ...
                AND postmeta_count.meta_key = '_Bestwebsite_casts'
                AND postmeta_ratings.meta_key = '_Bestwebsite_ratings'
        ";
